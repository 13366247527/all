<template>
  <div class="container messageboard">
    ...
    <div class="con-box">
      ...
      <div class="num_list">
        <span class="name">今日平均用时：</span><span class="num">{{addMap.totalAvgTime}}</span>
         <span class="name">托盘平均用时：</span><span class="num">{{addMap.palletAvgTime}}</span>
          <span class="name">人力平均用时：</span><span class="num">{{addMap.personAvgTime}}</span>
      </div>
      ...
      :index="indexMethod" label="序号" align="center">
          ...
          <el-table-column prop="sortNum" label="效率排名" min-width="15%" align="center" sortable></el-table-column>
          <el-table-column prop="vehgrpnam" label="物流商" align="center" min-width="25%" sortable></el-table-column>
          <el-table-column prop="totalNum" label="今日作业车数" align="center" min-width="15%"  sortable></el-table-column>
          <el-table-column prop="palletNum" label="托盘发货车数" align="center" min-width="15%" sortable></el-table-column>
          <el-table-column prop="totalwgt" label="重量(吨)" align="center" min-width="15%" sortable></el-table-column>
          <el-table-column prop="avgTime" label="厂内平均时间" align="center" min-width="15%" sortable></el-table-column>
        </el-table>
      </div>
      <el-pagination
        background
...
import { getVehgrpnam, getVehEfficiency, exportVehEfficiency } from "../../apis/apis";
...
export default {
  ...
  methods: {
    ...
    onSearch(val) {
      ...
      getVehEfficiency(data).then(data => {
    ...
    outExcel(val) {
      ...
      data.append("startTime", dateTimeStar ? dateTimeStar : "");
      data.append("endTime", dateTimeEnd ? dateTimeEnd : "");
      data.append("carType", 2);
      data.append("vehgrpnum", this.vehgrpnum);

      exportVehEfficiency(data).then(res => {
        this.listLoading = false;
