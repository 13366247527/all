  ...
  <div>
    ...
    <div class="shopTest">
        <ul>
			          ...
			          <li>
			             <span class="num">单号：</span>
                          <span>{{prdplndocNum}}</span>
			          </li>
        ...
        <mt-popup v-model="popupVisible"  class="mint-popup">
            <ul class="mint-popup-ul">
           	  ...
           	  <li class="mint-popup-tab">
           	  	<span v-for="(v,i) in tabText" :key="i" :class="textIndex==i?'textred':''" class="tab" @click="isOk(i,nowIndex)">{{v}}</span>
           	  </li>
           </ul>
        </mt-popup>
    </div>
       ...
       let params = {
        ...
        this.$http.post('wms/product/control/detail',params).then(res=>{
           ...
           if(res.data.ret_code == '0'){
             ...
             this.prdplndocNum = res.data.data.docNum
