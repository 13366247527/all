    ...
    <div class="tab pageFlex">
      <div class="commonTabDiv pd48">
        <ul class="commonTab common-content-box">
          ...
          <li>
            <span>电话：</span>
            <span class="red">{{dataList.conprntel}}</span>
          </li>
          <li>
            <span>业代：</span>
          ...
          <div>
            <p class="selectBox wid90">
              <input
                ...
                @keyup="num(item.rcvartsuu,index)"
              >
            </p>
          </div>
          <div>
...
export default {
  ...
  methods: {
    ...
    num(newVal, index) {
      let totartwgt = 0;
      this.tempData.forEach((item, index) => {
        // newVal<item.number?'':item.rcvartsuu=item.number
        totartwgt += (item.shpartqty / 1 - item.rcvartsuu / 1) * item.artwgt;
      });
      ...
      if (this.tempData[index].rcvartsuu >= this.tempData[index].shpartqty) {
        this.tempData[index].rcvartsuu = this.tempData[index].shpartqty;
