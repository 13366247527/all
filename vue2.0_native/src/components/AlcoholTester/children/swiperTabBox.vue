...
import motorman from "./motorman";
export default {
  name: "order-content",
  components: { employee, motorman},
  data() {
    return {
      mySwipr: '',
      focusDoctorList: [],
  ...
  mounted() {
    this.mySwipr = new Swiper(".swiper-container", {
      initialSlide : this.$route.path === "/AlcoholTester/employee" ? 0 : this.$route.path === "/AlcoholTester/motorman" ? 1 : 0,
      //autoHeight : true
    });
    this.mySwipr.on("slideChange", () => {
      this.$root.eventHub.$emit("JjslideTab", this.mySwipr.activeIndex);
    });
    this.$root.eventHub.$on("JjchangeTab", index => {
